    ...
    <div style={styles.container}>
      ...
      <form onSubmit={handleJoin} style={styles.form}>
        ...
        <div style={styles.inputGroup}>
          ...
          <input type="text" placeholder="이름을 입력해주세요" value={userName} onChange={(e) => setUserName(e.target.value)} style={styles.input} />
        </div>
        ...
        <div style={styles.inputGroup}>
          ...
          <div style={styles.inlineGroup}>
            ...
            />
            <button type="button" style={styles.subActionButton} onClick={handleCheckId}>
              아이디 중복 확인
            </button>
            {isLoginIdChecked === 'confirmedValue' ? <p>통과</p> : isLoginIdChecked === 'wrongValue' ? <p>불통</p> : <p>아이디 확인이나 해라ㅋ</p>}
          </div>
        </div>
        ...
        <div style={styles.inputGroup}>
          ...
          <input type="password" placeholder="비밀번호를 입력해주세요" value={loginPw} onChange={(e) => setLoginPw(e.target.value)} style={styles.input} />
        </div>

        {/* 휴대폰 번호 입력 + 인증 버튼 */}
        <div style={styles.inputGroup}>
          <label style={styles.label}>
            휴대폰 번호<span style={styles.required}>*</span>
          </label>
          <div style={styles.inlineGroup}>
            <input type="text" placeholder="숫자만 입력해주세요" value={userPhone} onChange={(e) => setUserPhone(e.target.value)} style={{ ...styles.input, flex: 1, marginBottom: 0 }} />
            <button type="button" style={styles.subActionButton} onClick={handleRequestPhoneAuth}>
              휴대폰 번호 인증
            </button>
            {isPhoneNumAvailable === 'confirmedValue' ? <p>통과</p> : isPhoneNumAvailable === 'wrongValue' ? <p>불통</p> : <p>폰번 확인이나 해라ㅋ</p>}
          </div>
        </div>

        {/* 메시지 표시 (에러 및 성공) */}
        {msg && <p style={{ color: result ? '#5b86e5' : '#ff4d4f', fontSize: '13px', marginBottom: '16px' }}>{msg}</p>}
        {isPhoneVerified && (
